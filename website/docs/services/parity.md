...
| Key Vault (vaults) | Full | Full | Full | Full | Full (PATCH) | Soft-delete + purge, accessPolicies, checkNameAvailability. ARM coverage tracked by `tools/specport/checklists/keyvault.md` |
...
For services where coverage is being tracked against the official Azure
REST specs, the canonical inventory lives in
[`tools/specport/checklists/`](https://github.com/moabukar/miniblue/tree/main/tools/specport/checklists).
Each markdown file is generated by `go run ./tools/specport diff <service>`
and shows every operation in the spec marked **IMPLEMENTED**, **MISSING**,
or **EXTRA**. Open the file for a live coverage breakdown.

| Service | Checklist |
|---------|-----------|
| Key Vault | [`keyvault.md`](https://github.com/moabukar/miniblue/blob/main/tools/specport/checklists/keyvault.md) |
